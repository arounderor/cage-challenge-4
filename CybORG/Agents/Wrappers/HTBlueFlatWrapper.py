...
# )
from CybORG.Simulator.Scenarios.HTScenarioGenerator import (
    HTScenarioGenerator,
)
...
MAX_USER_HOSTS = HTScenarioGenerator.MAX_USER_HOSTS
MAX_SERVER_HOSTS = HTScenarioGenerator.MAX_SERVER_HOSTS
...
class BlueFlatWrapper(BlueFixedActionWrapper):
    ...
    def _get_init_obs_spaces(self):
        ...
        long_observation_components = (
            observation_head + (NUM_SUBNETS * observation_middle) + observation_tail
        )
    ...
    def _build_comms_policy(self):
        network = self._build_comms_policy_network()
        comms_policy = network
        return comms_policy

    #TODO:待调整
    def _build_comms_policy_network(self):

        hosts = self.env.environment_controller.state.subnets.values()
        hosts = tuple(hosts)
